	...
	public boolean onCommand(
		...
		CompletableFuture.runAsync(() -> {
			...
			final List<ItemStat> itemStats = statPlayers.stream()
														.flatMap(statPlayer -> statPlayer.getItemStats().stream())
														.toList();
			...
			new BukkitRunnable() {
				@Override
				public void run() {
					writeToFile(generalStatsFile, generalStatsSql.toString());
					writeToFile(itemStatsFile, itemStatsSql.toString());
					writeToFile(mobStatsFile, mobStatsSql.toString());
				}
			}.runTaskAsynchronously(StatsBukkitPlugin.getInstance());
		});
		/*
